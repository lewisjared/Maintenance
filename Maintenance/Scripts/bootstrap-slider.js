...
(function( $ ) {
	...
	var Slider = function(element, options) {
		...
		if (parent.hasClass('slider') === true) {
			...
			this.picker = $('<div class="slider">'+
								'<div class="slider-track">'+
									...
									'<div class="slider-handle min-slider-handle"><div class="slider-text slider-text-handle"></div></div>' +
									'<div class="slider-handle max-slider-handle"></div>' +
									'<div class="slider-text slider-text-min"></div><div class="slider-text slider-text-max"></div>' +
								'</div>' +
								'<div class="slider-limit lower-limit">Lower</div>'+
								'<div class="slider-limit upper-limit"></div>'+
								'<div id="tooltip" class="tooltip hidden"><div class="tooltip-arrow"></div><div class="tooltip-inner"></div></div>'+
								'<div id="tooltip_min" class="tooltip hidden"><div class="tooltip-arrow"></div><div class="tooltip-inner"></div></div>' +
								'<div id="tooltip_max" class="tooltip hidden"><div class="tooltip-arrow"></div><div class="tooltip-inner"></div></div>' +
							'</div>')
		...
		this.textDiv = this.picker.find('.slider-text-handle');
		...
		//Calculate the true width available to the slider
		// and the correction factor required to convert percentages
		this.actualSize = this.size - this.handle1[0].clientWidth - this.handle1[0].offsetLeft * 2;
		this.widthCorr = this.actualSize / this.size;

		//Label the max and min values
		this.minText = this.picker.find('.slider-text-min');
		this.minText.text(self.min);
		this.minText[0].style[this.stylePos] = 0 + '%';
		this.maxText = this.picker.find('.slider-text-max');
		this.maxText.text(self.max);
		this.maxText[0].style[this.stylePos] = 100 * this.widthCorr + '%';
		...
		//Set the limit texts
		if (self.upper != 0)
			this.picker.find('.upper-limit').text(self.upper);
		if (self.lower != 0)
			this.picker.find('.lower-limit').text(self.lower);
	...
	Slider.prototype = {
		...
		layout: function(){
			...
			this.handle1Stype[this.stylePos] = positionPercentages[0] * this.widthCorr + '%';
			...
			if (this.orientation === 'vertical') {
				...
				this.selectionElStyle.left = Math.min(positionPercentages[0], positionPercentages[1])*self.widthCorr+'%';
			...
			if (this.range) {
				...
				this.tooltip[0].style[this.stylePos] = positionPercentages[0] * this.widthCorr + '%';
				if (this.orientation === 'vertical') {
					this.tooltip.css('margin-top', -this.tooltip.outerHeight() / 2 + 'px');
				} else {
					this.tooltip.css('margin-left', -this.tooltip.outerWidth() / 2 + 'px');
